...
def get_args():
    ...
    parser.add_argument('--input-path', '-ip', type=str, default=r'C:\Users\12828\Desktop\osteosarcoma\bone tumor data',
                        help='datasets path')
    parser.add_argument('--store-path', '-sp', type=str, default=r'features_dataset_t1+t2_cox(2022.10.17).csv',
...
if __name__ == '__main__':
    ...
    trainset_result.to_csv(os.path.join(args.father_path, 'cox_trainset(2022.09.29).csv'), index=False)
    testset_result.to_csv(os.path.join(args.father_path, 'cox_testset(2022.09.29).csv'), index=False)
