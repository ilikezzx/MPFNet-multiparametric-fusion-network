...
class MultiTransBlock(nn.Module):
    def __init__(self, in_ch, heads, num_block, size=28, attn_drop=0., proj_drop=0., rel_pos=True):
        super(MultiTransBlock, self).__init__()
        block = []
        for i in range(num_block):
            block.append(BasicTransBlock(in_ch, size, heads, attn_drop, proj_drop, rel_pos))
...
class BasicTransBlock(nn.Module):
    ...
    def __init__(self, in_ch, size, heads, attn_drop=0., proj_drop=0.,rel_pos=True):
        super().__init__()
        self.bn1 = nn.BatchNorm2d(in_ch)

        self.attn = LinearAttention(in_ch, size=size, heads=heads, dim_head=in_ch // heads, attn_drop=attn_drop,
                                    proj_drop=proj_drop, rel_pos=rel_pos)
...
class LinearAttention(nn.Module):
    ...
    def __init__(self, dim, size=28, heads=4, dim_head=64, attn_drop=0., proj_drop=0., rel_pos=True):
        super().__init__()
        ...
        if self.rel_pos:
            ...
            self.relative_position_encoding = RelativePositionBias(heads, size, size)
    ...
    def forward(self, x):
        ...
        if self.rel_pos:
            ...
            print(q_k_attn.shape, relative_position_bias.shape)
